...
del_int_kl1 = sensors4plumes:::deleteSensor(simulations = radioactivePlumes_local,
...
# apply cost function in 'deleteSensor'
del_int_kl1 = sensors4plumes:::deleteSensor(simulations = radioactivePlumes_area,
...
# apply cost function in 'deleteSensor'
del_minDist_1 = sensors4plumes:::deleteSensor(simulations = SimulationsSmall,
                    costFun = minDist,
...
# apply cost function in 'deleteSensor'
del_krigVar_a1_1 = sensors4plumes:::deleteSensor(simulations = radioactivePlumes_area,
                             costFun = krigVar_a1,
                             locationsDeletable = locDel3,
                             locationsKeep = locKeep3)
...
radioactivePlumes_local@plumes$totalDose =
  summaryPlumes(radioactivePlumes_local, fun = sum, kinds = "finaldose")[[2]]
### number of locations where it can be detected
radioactivePlumes_local@plumes$nDetectable =
  summaryPlumes(radioactivePlumes_local, fun = sum, kinds = "detectable")[[2]]
### earliest possible detection of plume inside the area
radioactivePlumes_local@plumes$earliestDetection =
  summaryPlumes(radioactivePlumes_local, fun = min, kinds = "time", na.rm = TRUE)[[2]]
...
sDet_1 = singleDetection(simulations = radioactivePlumes_local,
                         locations = c(locDel2[-1], locKeep2))
# apply cost function in 'deleteSensor'
del_sDet_1 = sensors4plumes:::deleteSensor(simulations = radioactivePlumes_local,
...
mDet_1 = multipleDetection(simulations = radioactivePlumes_local,
                         locations = c(locDel2[-1], locKeep2))
# apply cost function in 'deleteSensor'
del_mDet_1 = sensors4plumes:::deleteSensor(simulations = radioactivePlumes_local,
...
eDet_1 = earlyDetection(simulations = radioactivePlumes_local,
                           locations = c(locDel2[-1], locKeep2))
# apply cost function in 'deleteSensor'
del_eDet_1 = sensors4plumes:::deleteSensor(simulations = radioactivePlumes_local,
                          costFun = earlyDetection,
                          locationsDeletable = locDel2,
                          locationsKeep = locKeep2)

# check result of 'deleteSensor'
costWithout_eDet_1 = numeric(length(locDel2))
for (i in seq(along = locDel2)){
  costWithout_eDet_1[i] = earlyDetection(radioactivePlumes_local, c(locDel2[-i], locKeep2))[[1]]
}
expect_equal(
  del_eDet_1[[3]],
  costWithout_eDet_1)
expect_equal(
  del_eDet_1[[2]],
  which(costWithout_eDet_1 == min(costWithout_eDet_1))
)
expect_true(
  is.element(setdiff(locDel2, del_eDet_1[[1]]),
             locDel2[which(costWithout_eDet_1 == min(costWithout_eDet_1))])
)
