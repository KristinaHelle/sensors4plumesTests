...
test_that("coerce: SpatialPolygridDataFrame -> SpatialGridDataFrame", {
  ...
  expect_equivalent(
    ...
    spatialGridDataFrameB1@data[,1:length(spatialGridDataFrame1@data)])
})
...
test_that("coerce: SpatialPointsDataFrame {irregular} -> SpatialGridDataFrame", {
  ...
  expect_gt(SPolygridDFB3@bbox[1,2],
                   spatialPointsDataFrame2@bbox[1,2] - 0.5 * tol)
  expect_gt(SPolygridDFB3@bbox[2,2],
                   spatialPointsDataFrame2@bbox[2,2] - 0.5 * tol)
  expect_lt(SPolygridDFB3@bbox[1,1],
                   spatialPointsDataFrame2@bbox[1,1] + 0.5 * tol)
  expect_lt(SPolygridDFB3@bbox[2,1],
  ...
  expect_equivalent(
    ...
    spatialGridDataFrameB2[i,j]@data[,1:length(spatialPointsDataFrame2@data)]
  )
