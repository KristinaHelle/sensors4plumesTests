...
test_that("errorInterpolationFunctions", {
  ...
  expect_equal(absError(x = X),
               abs(diff(X)))
  expect_equal(delineationError(x = X, threshold = 1), 1)
  expect_equal(delineationError(x = rev(X), threshold = 1), 5)
  expect_equal(delineationError(x = X, threshold = 0.5), 0)
})
...
test_that("interpolationError", {
  ...
  fun_Rpl_mean3 = function(x, nout = 3, weight_l = 1, weight_p = 1){
    c(mean(x[,1], na.rm = TRUE),
      mean(x[,2], na.rm = TRUE),
      mean(x[,3], na.rm = TRUE))
  }
  fun_Rpl_mean1 = function(x, nout = 1, weight_l = 1, weight_p = 1){
    c(mean(x[,1], na.rm = TRUE))
  }
  ...
  expect_equal(# "cost"
    ...
    fun_Rpl_mean1(getValues(interpolationError1[["error_locationsplumes"]]))
  )
...
#  )
  ...
  # works only if delineation error returns 3 values
  ...
  expect_equal(# 'costMap'
    interpolationError1[["costLocations"]][i],
    mean(interpolationError1[["error_locationsplumes"]][i,][,1])
  )
