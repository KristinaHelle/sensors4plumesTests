...
#path1 = paste0(path.package("sensors4plumesTests"), "/tests")
...
values_i_1 = raster(x = matrix(1:45, nrow = 9),
                    crs = CRS("+init=epsg:4326"),
                    xmn = -90, xmx = 90, ymn = -90, ymx = 90)
simulations_i = Simulations(plumes = plumes_i, locations = locations_i, values = values_i_1)
...
values_ii_3 = raster(paste0(path.package("sensors4plumesData"), # incorrect form for Simulations
...
test_that("changeSimulationsPath",{
  ...
  expect_equivalent(simulations1b, simulations1B)
