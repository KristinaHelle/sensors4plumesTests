...
test_that("summaryLocations",
{
  ...
  summaryLocations_2 = summaryLocations(radioactivePlumes_area, fun = fun1, kinds = 1)
  expect_equivalent(
  ...
  summaryLocations_6 <- summaryLocations(radioactivePlumes_area, fun = sum,
                                        locations = c(locations, -2, 0, 3000, NA), kinds = 1, na.rm = FALSE))
  expect_true(
  ...
  expect_warning(summaryLocations_9 <- summaryLocations(radioactivePlumes_area, fun = sum, kinds = 1,
                                                      plumes = c(plumes, -2, 0, 2324, NA, plumes), na.rm = FALSE))
  expect_true(
    all(is.na(summaryLocations_9[[2]]))
  )
  expect_warning(summaryLocations_10 <- summaryLocations(radioactivePlumes_area, fun = sum, kinds = 1,
                                                        plumes = c(plumes, -2), na.rm = TRUE))
  expect_warning(summaryLocations_11 <- summaryLocations(radioactivePlumes_area, fun = sum, kinds = 1,
                                                         plumes = c(NA, 0, 10000, plumes), na.rm = TRUE))
  expect_equivalent(
    summaryLocations_10,
    summaryLocations_11
  )
  ...
  # weight
  # weight (numeric)
  summaryLocations_12 = summaryLocations(radioactivePlumes_area, fun = sum, kinds = 1,
  ...
  # weight (character)
  summaryLocations_13 = summaryLocations(radioactivePlumes_area, fun = sum, kinds = 1,
                                       weight ="dist2origin")
  expect_equal(
