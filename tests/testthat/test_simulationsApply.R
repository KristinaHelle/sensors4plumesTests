...
subset_radioactivePlumes_local =  subset(x = radioactivePlumes_local, locations = locationsI, plumes = plumesI,
                                         valuesOnly = TRUE, overwrite = TRUE)
...
# ?? why should this throw an error, it is clear that weight is taken from locations / plumes
## weight used but without default -> error
fun_Rp8 = function(x, nout = 2, weight){# how many of the models have Rsq < 0.5 (and slope <1)?
  c(mean(x[,3] <= 0.5, na.rm = TRUE) * weight[,2],
    mean(x[,2] <= 1, na.rm = TRUE))
}
fun_Rl7 = function(x, nout = 2, y , weight){# extra parameter, without default -> warning
  c(0.5 * mean(y[,1], na.rm = TRUE),
    mean(x[,1], na.rm = TRUE)/ weight[,2])
}
#expect_warning(
  simulationsApply__12a <- simulationsApply( # nout > 1, chunks
    simulations = radioactivePlumes_local,
    locations = locationsI,
    plumes = plumesI,
#    fun = fun2, # from sA_fun3
...
#)
# warnings not printed (but registered by expect_warning)
